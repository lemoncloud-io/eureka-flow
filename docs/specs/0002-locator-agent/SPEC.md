...
> [`0001-agent-chat`](../0001-agent-chat/SPEC.md): it reuses the same `LlmGateway`, `ToolExecutor`,
...
1. **The agent is the sole editor.** The canvas is read-only to the human (owner included); all
   ...
   "single editor" concern from 0001 collapses to a guarantee (§3).
2. **Direct apply — no approval gate.** This version deliberately skips the plan/accept step; a
   `move_node` call takes effect at once. (The approval machinery from 0001 is deferred, §9.)
...
- **Out (→ 0001 or §9):** add / delete / reconfigure / connect nodes; the draft + plan + Accept/Reject
...
Same cast as 0001, minus the Workspace (there is no draft — the move is applied live).
...
| **LlmGateway**    | The one outbound LLM dependency — a dev **offline** gateway (no network, no key) + a fake for tests. Reused from 0001. |
| **ToolExecutor**  | Runs the agent's tool calls: validate args → route by name → do it → result. Reused from 0001.                         |
| **CanvasBinding** | The single door to the live canvas: `readGraph()` to find the node, `updateNode()` to move it. Built.                  |
| **Storage**       | The persisted session (`SessionState`) the Panel renders from. Reused from 0001.                                       |

The `grant` / effective-set machinery of 0001 **is** live: the executor checks each tool's required
capability against the agent's grant (the mutate tool requires `canModifyCanvas`). Only 0001's
...
[0001 §5](../0001-agent-chat/SPEC.md#5-interface-definitions). `CanvasBinding` / `Graph` / `XY` are now
...
The locator agent is one `AgentConfig` (0001 §5): persona "relocate the user's nodes"; `grant` =
...
per call — validate args → check the capability → dispatch. Other agents reuse the same provider;
...
flow-edit agent (0001). If the user asks for one, the agent declines and points at what it _can_ do.

### 6.4 Session & storage

Reused from 0001 §6.6, with the generic `AgentPhase` narrowed to the locator's subset (no `plan` slot,
...
- **The Agent Panel is docked on the right and is always present** whenever a flow is open. There is
  **no open/close toggle** — the agent is a permanent part of the flow editor, because it is the only
  way to edit the canvas (§2.1). Opening a flow rehydrates its persisted session (0001 §6.6) so the
...
| Agent / LLM / tools | `Agent`, `LlmGateway`, `ToolExecutor`, `ToolProvider`, `SessionState`, `Storage`                            | [`0001-agent-chat` §7](../0001-agent-chat/SPEC.md#7-grounding-what-already-exists) |

New agent code lives in `libs/agent/src` (as proposed by 0001); the locator agent is its first
...
Each folds cleanly back into the 0001 design:
...
- **Approval gate** — route moves through the draft + plan + Accept/Reject flow of 0001 instead of
...
- **Session-role permission ceiling** — per-tool capability checks against the agent's `grant` already
  ship (§6.3); still deferred is clamping that grant to the session's role ceiling (0001).
...
- **Fold into the flow-edit agent** — once 0001 ships, `move_node` becomes one more mutate tool on the
  single flow-edit agent rather than a standalone agent.

---

Architecture this builds on: **[0001-agent-chat/SPEC.md](../0001-agent-chat/SPEC.md)**.
