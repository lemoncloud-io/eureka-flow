...
| Storage (`useAgentStore`)  | `libs/agent/src/store.ts` (Zustand)                             | Passive session store: messages, the turn `phase` (with any pending gate), and `pendingRunIntent`; persisted to localStorage. The Panel renders purely from it.                                                                                                                              |
