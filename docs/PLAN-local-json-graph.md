...
### 🟠 배포 전 DEV 스모크 — 한 번에 두 개를 막는 게이트 (S6 착륙 후 실행)

브랜치는 어차피 S6 전까지 배포 금지(🔴 실행 회귀)다. **그 게이트에서 아래를 같이 확인한다** — S5의 배선과 S6의 실행 게이트가 같은 스모크로 덮인다.

S5 배선은 **구성 + 단위로만 검증됨**(로드 4곳이 `captureBaseline`을 _옳은 인자로_ 부르는지는 인증 필요). `useSocketHandlers.ts:68`이 실제로 이 함정에 빠져 있었다 = 이 클래스의 버그가 여기 산다는 증거.

- [ ] **owner로 기존 플로우 로드 → 아무것도 안 건드림** → dirty 표시 없음 + 네트워크 탭에 `/save` 요청 **0회** (= S5 수용 기준. 실패하면 baseline 출처/시점이 틀린 것)
- [ ] 새로고침 → 위 반복 (auto-save on/off 양쪽)
- [ ] 노드 1개 이동 → 2초 후 `/save` **1회** (게이트가 반대로 막고 있지 않은지)
- [ ] 노드 실행 → 상태 변화 후 `/save` **0회** (D-B: 런타임 상태는 편집이 아님)
- [ ] `/editor` 신규 진입 → `/flows/0/save` **0회**, 첫 save 때만 발급 (AC1/AC2)
