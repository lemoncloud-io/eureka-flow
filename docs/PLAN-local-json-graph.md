...
| C12 | R6: "localStorage(용량 초과 대비 **IndexedDB 폴백**)"           | **거꾸로다. localStorage는 이미지 노드 1개에 터진다.** 업로드는 base64를 `config.imageData`에 **그대로** 넣는다(`utils/index.ts:300` → `WorkflowCanvas.tsx:1451`), S3 업로드 경로는 **존재하지 않음**(`s3://`는 전부 읽기 방향). 5MB 이미지 1개 → base64 ~6.7MB → **단일 노드가 5MB 쿼터 초과**. zip/텍스트는 **압축 안 함**(`index.ts:285,293`) → 58MB 파일 → base64 ~77MB. 이미지 압축도 soft cap(`imageCompression.ts:129`가 목표 미달이어도 `quality<0.5`면 탈출) | **IDB가 폴백이 아니라 기본.** base64는 작업본에서 **뺄 수 없다 — 그게 미저장 작업 그 자체다**        |
...
- [x] **S7 — R6: 로컬 지속성 + 오프라인** (P1) ✅ 2026-07-17 — **auto-save off 영속화 공백 닫힘**
    ...
    **정찰 결과 3개 — 요구사항 R6의 전제 2개가 틀렸다:**
    1. **C12: localStorage 불가.** 위 표 참조. 이미지 노드 1개(~6.7MB base64)가 5MB 쿼터를 혼자 넘긴다. zip/텍스트는 무압축 58MB → base64 ~77MB. 이미지 3개면 20MB+, zip 3개면 ~230MB. **base64를 draft에서 빼는 우회는 불가** — 그게 미저장 작업 자체이고, 빼고 복구하면 이미지 빠진 손상된 플로우가 된다. → **IDB 기본**(폴백 아님). 단 IDB는 **비동기**라 종료 시 flush가 best-effort (localStorage는 동기라 이게 유일한 장점)
    2. **beforeunload는 2026-05-06 `8e734ed`에서 louis가 의도적으로 제거함** (obs 10244: _"Removing this alert improves user experience by eliminating interruptions during page navigation and refreshes"_). 그때 그건 `serializeWorkflowState` 문자열 비교라 **런타임 상태로 항상 dirty** → 실행 후엔 새로고침마다 발화 = 그래서 짜증났던 것(D-B가 고친 바로 그 결함).
       → **분리해야 한다**: R6의 의도는 "작업 유실 방지"인데 **draft 지속성이 이미 그걸 한다**. 확인창을 되살리면 *일어나지도 않을 유실*을 막겠다고 louis가 지운 그 방해를 되돌리는 것. 반면 **디바운스 창 안의 편집은 아직 저장 안 됨** = 진짜 구멍 → **조용한 flush**(`preventDefault` 없음, 다이얼로그 없음)로 메운다. 모바일에선 `beforeunload`가 신뢰 불가 → **`visibilitychange`/`pagehide` 사용**
    3. **R6 4번째 불릿(save 실패 시 재시도)은 대부분 이미 있다**: 작업본 유지 ✅(S5 `rebaseline`이 성공 시에만 발화), `saveStatus='error'` ✅(`Header.tsx:236`), `retrySave` ✅(`FlowEditorPage:766 onRetrySave`). **없는 건 연결 복구 감지뿐**(`navigator.onLine`/`online` 리스너 코드베이스에 0건)

    **⚠️ draft가 튜토리얼을 삼키는 함정**: `TutorialPage`/`MobileTutorialPage`가 **같은 `useCanvasStore`**를 쓴다(`TUTORIAL_WORKFLOW`를 `loadWorkflow`). 전역 구독으로 draft를 쓰면 튜토리얼 노드가 draft로 저장되고 `currentFlowId=null`이라 **"미저장 새 플로우"로 다음 `/editor` 부팅 때 복구됨**. → draft writer는 **에디터 페이지가 명시적으로 켜는 훅**이어야 함(전역 구독 금지) + `permissions.canSave` 게이트

    **복구 시 baseline 출처 (또 baseline 출처 문제 — 매 슬라이스 반복되는 주제)**:
    - **온라인 + 기존 플로우**: 서버에서 로드했으므로 `captureBaseline`이 이미 **신선한** baseline을 잡았다 → 복구는 **작업본만** 되살린다. `draft.baseline`을 쓰면 **다른 세션의 서버 변경을 가린다**
...
### ✅ S3가 연 것 — 닫혔다. **DEV 실환경에서 확인됨 (2026-07-18)**. 아래는 원 기록

> **닫힘 (2026-07-17, S6 `483326a`)**: 사용자 발동 실행 5곳이 전부 `useRunGate`를 통과 — dirty면 save 후 실행, 거부/실패 시 중단. editor 구조 변경은 save로 못 고치므로 차단 + 경고.
> **DEV 스모크로 실증됨**: 신규 flow + 미저장 노드 → Run All → 게이트 확인창 → save(id 발급) → `run 200`, **404 없음**. 상세는 아래 "DEV 스모크 실행됨" 절.
...
### ✅ DEV 스모크 실행됨 (2026-07-18, 실키 uid 1000003, DEV 백엔드 `flw-d1`)

브라우저(Playwright) + curl로 진행. **🔴 실행 회귀는 실환경에서 닫힘을 확인.** 남은 미검증은 2번째 계정/오프라인 시뮬이 필요한 것뿐.

**서버 계약 (curl, `_api_/flows`):**

- [x] **AC3 ID 라운드트립** — 클라 hex id(`n`+32hex, `e`+32hex) 2노드 1엣지 저장 → 로드 → **id·config 전부 verbatim 생존**. flow 1010906
- [x] **C7 삭제 = 목록 누락** — N2 빼고 저장 → 로드 시 N2 없음, **`#` 툼스톤 없음**
- [x] **C8 `nodes:[]`** → 멤버십 비워짐 (로드 시 노드 0)

**프론트 배선 (브라우저, owner):**

- [x] **AC2 지연 생성** — `/editor` 신규 진입 부팅 시 `/flows/0/save` **0회** (프로필 GET 1회뿐)
- [x] 노드 추가 → 네트워크 **0회** (URL 해시에 클라 hex id)
- [x] **🔴 회귀 종료** — 신규 flow + 노드 → 저장 없이 Run All → **게이트 확인창** "Save your changes and run?" → 수락 → `POST /flows/0/save 200`(id 1010915 발급) → `/load 200` → **`POST /flows/1010915/run 200`** → `/nodes/n06c…:out/port 200`. **404 없음. 노드 RUNNING→COMPLETED.** auto-save OFF 기본 경로
- [x] **stale flowId 함정 닫힘** — run이 방금 발급된 1010915를 씀(조용한 no-op 아님)
- [x] **S5 baseline** — 로드된 flow에서 draft.baseline = 서버 상태(`Hello Eureka`), working = 편집 → dirty 정확
- [x] **S7 draft 기록** — config 편집 → 800ms 후 IDB에 draft(working=편집, baseline=서버). autosave OFF에도 기록됨
- [x] **S7 복구** — 새로고침 → 확인창 "…changes that were never saved. Restore them?" → 수락 → 편집값 복원(`Hello Eureka` 아님)
- [x] **S7 복구 거부** — 새로고침 → 거부 → 서버값 표시 + **draft IDB에서 삭제**
- [x] **run은 draft 안 만듦** — 실행 후 IDB `current` = null (런타임 상태는 편집 아님, D-B)
- [x] **C12 IDB 결정** — 6MB(이미지 크기) 값: `localStorage.setItem` → **QuotaExceededError**, IndexedDB → **accepted**. IDB 선택의 근거 실증
- [x] 앱 콘솔 에러 **0** (남은 콘솔 에러는 전부 인프라: OAuth refresh 타임아웃/누락 svg 아이콘/이전 세션 version.json 폴링)

**미검증 (실환경 제약, 배포 후/2번째 계정 필요):**

- [ ] **R5-b editor 구조 경고 (C11)** — 비소유자 editor의 구조 save 차단/경고. 내 키는 **모든 flow의 owner**라 재현 불가(DEV 역할칩은 클라 권한만 바꿈, 서버 `hasOwned` 불변). **다른 계정이 공유한 flow 필요.** 단 클라 분기는 `runGate.spec` 단위+뮤테이션으로 커버됨
- [ ] **오프라인 분기** (`draft.baseline` 폴백) + **재연결 토스트** — 네트워크 토글 시뮬 미실행
- [ ] **undo/redo** (S2부터 미검증, P2/S9 영역 — 내 슬라이스가 동작 안 바꿈)
- [ ] **모바일** Run All/복구 (데스크톱과 코드 공유하나 실기기 미확인)
- [ ] `visibilitychange` flush, Safari/Firefox 사생활모드 IDB 실패 graceful

**부산물**: DEV에 테스트 flow 2개 남음 (1010906, 1010915, owner=1000003). 삭제 미실행 — 무해
