...
| S2  | `observableCanvasBinding(fakeBinding, () => tracer).addNode('http', xy)`                                | inner `addNode` called once; one `canvas.mutate {op:'addNode', nodeId}`; returned id is inner's                                                |
...
| P3  | root `turn.start` graph `{nodes:[]}` + final root `turn.done` graph `{nodes:[n9]}` | `toGraphDiff` → `addedNodes:['n9']`, others empty; `before`/`after` intact                                                                                                             |
...
| #   | Given / When                                                             | Oracle                                                                                                                                                                                                                       |
| --- | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| I1  | orchestrator handles a request that spawns one builder which adds a node | records project to: a 2-node trace tree, two non-empty transcripts, and a `GraphDiff` whose `addedNodes` matches the binding mutation; every record carries `runId` + `flowPath`                                             |
