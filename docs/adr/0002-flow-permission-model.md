...
The server (`eureka-flows-api`) already distinguishes **ownership** (`hasOwned` = same `sid`+`uid`)
...
heuristic: the `/load` response carries both `hasOwned` and `isEditable` (required booleans), and
...
- **`hasOwned` + `isEditable` booleans on `/load` (chosen)** — the client composes roles from the
...
- **Client-only, no server change** — rejected: without `hasOwned` the client cannot tell Owner
