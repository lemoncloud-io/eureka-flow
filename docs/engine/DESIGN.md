...
| `CanvasBinding` 을 엔진 위에 **재구현**하고 툴 확장(`addNode`/`connect`…) | **절반.** 인터페이스는 그대로, 구현체는 엔진 위로 옮겼다 — `createEngineCanvasBinding` 이 `libs/agent` 에서 `FlowEngine` 을 직접 감싸고 데스크톱 전용 `WorkflowCanvasRef` 바인딩은 지웠다(이제 모바일·헤드리스도 같은 바인딩). 툴 확장은 아직 | `docs/browser-agent/docs/canvas-binding.md` |
