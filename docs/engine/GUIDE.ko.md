...
한 장짜리 지도. · English: [GUIDE.md](GUIDE.md)

> 두 판은 같은 내용이다. 고칠 때 **양쪽 다** 고쳐라 — 한쪽만 고치면 다른 쪽이 조용히 거짓이 된다.
> 배포된 npm README 가 링크하는 건 영어판(`GUIDE.md`)이다.
...
| `docs/engine/PLAN.md`   | Phase 0~6 실행 계획, 불변식, 결함 재현 절차, 정정 이력. 배포 결정은 §14                            |
| `libs/engine/README.md` | **`@lemoncloud/flow-engine`** — npm 페이지 본문. 레포 밖 소비자가 처음 보는 한 장                  |
