...
function getProjectsToProcess() {
    ...
    if (args.length === 0) {
        // 모든 존재하는 프로젝트 반환
        return Object.keys(CONFIG.projectPaths).filter(project => {
            const projectPath = path.resolve(process.cwd(), CONFIG.projectPaths[project]);
            return fs.existsSync(projectPath);
        });
    }

    // 유효한 프로젝트만 필터링
    return args.filter(project => {
        if (!CONFIG.projectPaths[project]) {
            console.error(`Warning: Invalid project "${project}". Skipping.`);
            return false;
        }
        const projectPath = path.resolve(process.cwd(), CONFIG.projectPaths[project]);
        if (!fs.existsSync(projectPath)) {
            console.error(`Warning: Project "${project}" does not exist. Skipping.`);
            return false;
        }
        return true;
    });
...
function main() {
    try {
        ...
        if (projectsToProcess.length === 0) {
            console.log('No valid projects to process');
            return;
        }
...
if (process.argv.length > 4) {
    ...
    console.log('  If no project is specified, all existing projects will be updated');
    process.exit(1);
}
