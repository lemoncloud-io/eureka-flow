...
import { NodeData, PortDefinition, ConfigField, LogEntry, DataPacket } from '../types';
...
interface PortItemProps {
    ...
    onMouseDown: (nodeId: string, portId: string, type: 'input' | 'output', portType: string, e: React.MouseEvent) => void;
...
const PortItem: React.FC<PortItemProps> = ({ port, type, nodeId, hasData, isHighlighted, onMouseDown, onMouseUp }) => {
    return (
        <div 
            ...
            onMouseUp={(e) => { e.stopPropagation(); onMouseUp(nodeId, port.id, type, port.type); }}
        >
             {type === 'input' && (
                <div 
                    ...
                    `}
                    onMouseDown={(e) => { e.stopPropagation(); onMouseDown(nodeId, port.id, type, port.type, e); }}
                />
            )}
            <span className={`text-[10px] uppercase tracking-wider font-semibold select-none transition-colors ${isHighlighted ? 'text-yellow-400' : 'text-gray-400'}`}>
                {port.label}
            </span>
            {type === 'output' && (
                <div 
        ...
        case 'file':
            return (
                <div className="mt-3 flex flex-col items-center">
                    ...
                    {value ? (
                        <img src={value} className="w-full h-32 object-contain rounded mb-2 border border-gray-700 bg-black/50" alt="Preview" />
                    ) : (
    ...
    'debug-log': ({ node }) => {
        ...
        return (
            <div className="mt-2 p-2 bg-black/50 rounded border border-gray-800 text-green-400 font-mono text-[10px] break-all max-h-24 overflow-y-auto">
                {lastInput !== undefined ? (typeof lastInput === 'object' ? JSON.stringify(lastInput, null, 2) : String(lastInput)) : <span className="text-gray-600">Waiting for data...</span>}
            </div>
        );
    },
    'preview': ({ node }) => {
        ...
        if (!lastInput) return <div className="mt-2 text-xs text-gray-500 italic text-center py-4 bg-gray-900/30 rounded border border-dashed border-gray-700">Waiting for data...</div>
...
interface NodeBlockProps {
  ...
  onPortMouseDown: (nodeId: string, portId: string, type: 'input' | 'output', portType: string, e: React.MouseEvent) => void;
...
export const NodeBlock: React.FC<NodeBlockProps> = ({
  ...
  const STATUS_VISUALS = {
      RUNNING: {
          ...
          icon: <div className="absolute inset-0 border-2 border-yellow-500 border-t-transparent rounded-full animate-spin"></div>,
  ...
  const ConfigModal = () => createPortal(
    <div 
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-lg shadow-2xl w-[400px] max-w-[90vw] max-h-[80vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-gray-800 bg-gray-800/50">
                <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded bg-gray-800 flex items-center justify-center border border-gray-600 text-[10px]">⚙️</div>
                    <div>
            ...
            <div className="p-4 overflow-y-auto flex-1">
                ...
                <div className="mb-4 pb-4 border-b border-gray-800">
                    <label className="text-xs text-gray-400 block mb-1 font-semibold">Custom Label (Note)</label>
                    <input
                        type="text"
                        className="w-full bg-gray-950 border border-gray-700 rounded p-2 text-sm text-white focus:border-blue-500 outline-none"
                        value={tempLabel}
                ...
                {configSchema.length === 0 ? (
                    <div className="text-center text-gray-500 py-8 text-xs italic">No configurable settings.</div>
                ) : (
            ...
            <div className="p-4 border-t border-gray-800 bg-gray-800/30 flex justify-end">
                <button onClick={() => setShowConfig(false)} className="bg-blue-600 hover:bg-blue-500 text-white text-xs px-4 py-2 rounded transition-colors font-semibold">Done</button>
            </div>
        </div>
    </div>, document.body
  );
  ...
  return (
    <>
        <div
            ...
            onDoubleClick={(e) => { e.stopPropagation(); setShowConfig(true); }}
        >
            {/* Header */}
            <div className={`${visuals.header} p-2 rounded-t-md flex justify-between items-center border-b border-gray-700/50 cursor-move h-10 box-border transition-colors duration-300`}>
                
                {/* Title Section (with Inline Edit) */}
                {isEditingLabel ? (
                    <div className="flex-1 mr-2" onMouseDown={(e) => e.stopPropagation()} onDoubleClick={(e) => e.stopPropagation()}>
                        <input
                            ref={labelInputRef}
                            type="text"
                            value={tempLabel}
                    ...
                    >
                        <div className="w-3 h-3 mr-1 relative flex items-center justify-center shrink-0">{visuals.icon}</div>
                ...
                <div className="flex items-center gap-1 relative shrink-0">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setShowMenu(!showMenu); }} className="text-gray-400 hover:text-white w-6 h-6 flex items-center justify-center rounded hover:bg-white/10">⋮</button>
                    {showMenu && (
                        <>
                            ...
                            <div className="absolute right-0 top-7 w-32 bg-gray-800 border border-gray-700 rounded shadow-xl z-50 flex flex-col py-1 animate-in fade-in zoom-in-95 duration-100">
                                 <button onClick={(e) => { e.stopPropagation(); setShowMenu(false); setIsEditingLabel(true); }} className="text-left px-3 py-2 text-xs text-gray-200 hover:bg-gray-700 hover:text-white flex items-center gap-2"><span>✏️</span> 이름 변경</button>
                                 <button onClick={(e) => { e.stopPropagation(); setShowMenu(false); onViewLogs(); }} className="text-left px-3 py-2 text-xs text-gray-200 hover:bg-gray-700 hover:text-white flex items-center gap-2"><span>📜</span> 로그 보기</button>
                                 <button onClick={(e) => { e.stopPropagation(); setShowMenu(false); setShowConfig(true); }} className="text-left px-3 py-2 text-xs text-gray-200 hover:bg-gray-700 hover:text-white flex items-center gap-2"><span>⚙️</span> 설정 보기</button>
                            </div>
                        </>
                    )}
                    <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="text-gray-500 hover:text-red-400 w-6 h-6 flex items-center justify-center rounded hover:bg-white/10">✕</button>
                </div>
            </div>

            {/* Body */}
            <div className="p-3 pb-6"> {/* Add extra padding at bottom to prevent overlap with footer stats */}
                <p 
                ...
                <div className="border-t border-gray-700/50 mt-1 pt-1">
                    ...
                    {VISUALIZATION_COMPONENTS[node.type] && React.createElement(VISUALIZATION_COMPONENTS[node.type], { node })}
                </div>
                ...
                <>
                    ...
                    {node.status === 'RUNNING' && (
                        <div className="absolute bottom-0 left-0 w-full h-1 bg-gray-700 rounded-b-md overflow-hidden">
                            <div className="h-full bg-blue-500 transition-all duration-300 ease-out" style={{ width: `${node.executionStats?.progress || 0}%` }}/>
                        </div>
                    )}
