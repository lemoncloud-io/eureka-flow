...
import { WorkflowState, LogEntry, BlockDefinition, DataPacket, ListResult, BlockView, ProcessResult, ProcessBody } from '../types';
...
async function saveFlow(state: WorkflowState, name: string): Promise<{ success: boolean, id: string }> {
    ...
    try {
      ...
      let existing = list.find(f => f.name === name);
      let id = existing ? existing.id : Math.random().toString(36).substr(2, 9);
      ...
      for (let i = 0; i < count; i++) {
          ...
          randomLogs.push({
              ...
              message: isError 
                ...
                : (isWarn ? `High latency detected during step ${i}` : `Successfully processed chunk ${i}`)
          });
      }
...
const MOCKED_BLOCK_DEFINITIONS: BlockDefinition[] = [
  // 1. User Text Input
  {
    ...
    configSchema: [
      { key: 'text', type: 'text', label: 'Value' }
    ],
  ...
  // 2. User Image Input
  {
    ...
    configSchema: [
      { key: 'imageData', type: 'file', label: 'Image' }
    ],
    execute: async (inputs, config, onProgress) => {
  ...
  // 3. Buffer / Delay
  {
    ...
    configSchema: [
      { key: 'delayMs', type: 'number', label: 'Delay (ms)' }
    ],
    execute: async (inputs, config, onProgress) => {
  ...
  // 5. Validation Block
  {
    ...
    configSchema: [
      { key: 'minLength', type: 'number', label: 'Error if Length <=' }
    ],
    execute: async (inputs, config, onProgress) => {
      const text = String(inputs['in'].value);
  ...
  // 9. Workflow Component
  {
    ...
    configSchema: [
      { key: 'selectedFlowId', type: 'workflow-selector', label: 'Select Workflow' }
    ],
    execute: async (inputs, config, onProgress) => {
...
async function listBlocks(): Promise<BlockDefinition[]> {
    ...
    const list = $res?.data?.list?.map(N => N?.$definition).filter(N => !!N?.label).map(N => {
    ...
    return list.reduce<BlockDefinition[]>((L, N) => {
