...
import type { Agent, CanvasBinding, LlmGateway, SessionState, Storage } from '@flows/agent';
...
interface UseLocatorAgentResult {
...
/**
 * React binding for the locator {@link createLocatorAgent}: it owns a per-flow session store
 * (backed by localStorage, so it survives a page reload / re-opening the flow — §6.4) and
 * re-renders the panel on every `save` — the one-way render loop from the spec (Panel emits
 * `send` → the agent writes `SessionState` → store → Panel; §6.4).
 *
 * The agent (and its storage) are rebuilt when the flow changes. Crucially, the outgoing
 * agent is **aborted and silenced** when it is replaced or the panel unmounts, so an
 * in-flight turn from flow A can't mutate flow B's canvas or clobber its transcript, and
 * the LLM stream is freed.
 */
export const useLocatorAgent = ({ binding, flowId, gateway }: UseLocatorAgentArgs): UseLocatorAgentResult => {
    ...
    const instance = useMemo<AgentInstance>(() => {
        ...
        const agent = createLocatorAgent({ gateway, binding, storage, flowId });
        return {
            agent,
            arm: () => {
                alive = true;
