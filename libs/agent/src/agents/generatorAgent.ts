...
export const GENERATOR_SYSTEM_PROMPT = [
    ...
    '- `temperature` / `topK` / `topP` tune sampling (higher temperature = more random). `systemPrompt` is the',
    '  standing instruction; `prompt` is the per-run user prompt — keep the two distinct.',
    '',
    'How to work:',
