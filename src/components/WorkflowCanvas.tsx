...
import { NodeData, Connection, DataPacket, WorkflowState, LogEntry } from '../types';
        ...
        >
            <div className="bg-gray-950 border border-gray-700 rounded-lg shadow-2xl w-[600px] max-w-[95vw] h-[500px] flex flex-col animate-in fade-in zoom-in-95 duration-200 font-mono">
                ...
                <div className="p-2 border-b border-gray-800 bg-gray-900/50 flex gap-2">
                    <input type="text" placeholder="Filter logs..." className="flex-1 bg-black border border-gray-700 rounded px-2 py-1 text-xs text-white focus:border-blue-500 outline-none" value={filter} onChange={(e) => setFilter(e.target.value)} />
                    <button onClick={() => { setLoading(true); api.fetchBlockLogs(nodeId).then(data => { setLogs(data); setLoading(false); }); }} className="px-3 bg-gray-800 hover:bg-gray-700 border border-gray-700 rounded text-xs text-gray-300">Refresh</button>
                </div>
                <div className="flex-1 overflow-y-auto p-2 space-y-1 bg-black/30">
                    {loading ? <div className="flex justify-center items-center h-full text-gray-500 text-xs">Loading...</div> : 
                     filteredLogs.length === 0 ? <div className="text-center text-gray-600 text-xs py-10 italic">No logs found.</div> : 
                     filteredLogs.map(log => (
                        <div key={log.id} className="text-[11px] flex gap-2 p-1.5 hover:bg-white/5 rounded border-b border-gray-800/50 last:border-0">
                            <div className="text-gray-500 w-24 shrink-0 font-mono">{new Date(log.timestamp).toLocaleTimeString()}</div>
                            <div className={`w-14 shrink-0 font-bold ${log.level === 'ERROR' ? 'text-red-500' : log.level === 'WARN' ? 'text-yellow-500' : 'text-blue-400'}`}>{log.level}</div>
...
export const WorkflowCanvas = forwardRef<WorkflowCanvasRef, WorkflowCanvasProps>(({ readOnly, initialData, onNodeSelect, onChange }, ref) => {
  // --- STATE ---
  ...
  const [dragState, setDragState] = useState<{ nodeId: string; startX: number; startY: number; initialX: number; initialY: number } | null>(null);
    ...
    addNode: (type: string) => {
        ...
        if (newDef.inputs.length > 0) {
            ...
            const findCompatibleOutput = (n: NodeData) => {
                ...
                return def.outputs.find(out => 
                   out.type === firstInput.type || out.type === 'any' || firstInput.type === 'any'
                );
            };
  ...
  // --- ENGINE LOGIC ---
  const propagateOutputs = useCallback((sourceNodeId: string, outputs: any) => {
    ...
    setNodes(prevNodes => {
        ...
        const nextNodes = prevNodes.map(node => {
            ...
            let newInputData = { ...node.inputData };
  ...
  const executeNode = useCallback(async (nodeId: string, manualOverrideInputs?: any) => {
    if (readOnly) return;
    ...
    if (!nodeDef) {
        setNodes(prev => prev.map(n => n.id === nodeId ? { ...n, status: 'ERROR', errorMessage: 'Unknown Block Type' } : n));
        return;
    }
  ...
  const handleToggleAuto = (nodeId: string) => {
      if (readOnly) return;
      saveCheckpoint();
      setNodes(prev => prev.map(n => n.id === nodeId ? { ...n, autoExecutionEnabled: !n.autoExecutionEnabled } : n));
  };
  ...
  const handleNodeMouseDown = (e: React.MouseEvent, nodeId: string) => {
    ...
    if (['INPUT', 'SELECT', 'TEXTAREA', 'BUTTON', 'LABEL'].includes(target.tagName) || target.isContentEditable) {
        return;
    }
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (dragState && !readOnly) {
        ...
        setNodes(prev => prev.map(n => n.id === dragState.nodeId ? { 
            ...n, 
  ...
  const handleMouseUp = () => {
    ...
    if (dragState && dragStartSnapshotRef.current) {
        ...
        if (currentNode && originalNode && (currentNode.position.x !== originalNode.position.x || currentNode.position.y !== originalNode.position.y)) {
  ...
  const handlePortMouseDown = (nodeId: string, portId: string, type: 'input' | 'output', portType: string, e: React.MouseEvent) => {
      if (readOnly) return;
      handleSelectionChange(nodeId);
      setSelectedConnectionId(null);
      if (type === 'output') {
          const worldPos = screenToWorld(e.clientX, e.clientY);
          setConnectionDraft({ sourceNodeId: nodeId, sourcePortId: portId, sourceType: portType, mouseX: worldPos.x, mouseY: worldPos.y });
      }
  };

  const handlePortMouseUp = (targetNodeId: string, targetPortId: string, type: 'input' | 'output', targetType: string) => {
      if (readOnly) return;
      if (connectionDraft && type === 'input') {
          const sourceNode = nodes.find(n => n.id === connectionDraft.sourceNodeId);
          const targetNode = nodes.find(n => n.id === targetNodeId);
          if (sourceNode && targetNode && isValidConnection(sourceNode, 0, targetNode, 0, connectionDraft.sourceType, targetType)) {
              saveCheckpoint();
              setConnections(prev => {
                  const filtered = prev.filter(c => !(c.targetNodeId === targetNodeId && c.targetPortId === targetPortId));
                  const newConn = { 
                      id: generateId(), 
              ...
              if (sourceNode.outputData[connectionDraft.sourcePortId]) {
                   setNodes(prev => prev.map(n => n.id === targetNodeId ? { ...n, inputData: { ...n.inputData, [targetPortId]: sourceNode.outputData[connectionDraft.sourcePortId] } } : n));
              }
          }
      }
      setConnectionDraft(null);
  ...
  const getPortPosition = (nodeId: string, portId: string, type: 'input' | 'output') => {
      ...
      const portIndex = type === 'input' ? def.inputs.findIndex(p => p.id === portId) : def.outputs.findIndex(p => p.id === portId);
        ...
        if (isCtrlOrCmd && e.key.toLowerCase() === 'v') {
            if (clipboard) {
                ...
                const newNode: NodeData = {
                    ...clipboard,
                    id: generateId(),
                    position: { x: Math.round(x / GRID_SIZE) * GRID_SIZE, y: Math.round(y / GRID_SIZE) * GRID_SIZE },
                    status: 'IDLE',
                    inputData: {},
                    outputData: {},
                    errorMessage: undefined,
  ...
  }, [nodes, selectedNodeId, selectedConnectionId, hoveredConnectionId, clipboard, readOnly, viewport, saveCheckpoint, deleteNode, handleSelectionChange]);
  ...
  const detailConnection = selectedConnectionId ? connections.find(c => c.id === selectedConnectionId) || null : null;
        ...
        >
            ...
            >
                <svg className="absolute overflow-visible top-0 left-0 w-full h-full">
                    ...
                    {connectionDraft && (
                        <ConnectionLine 
                            x1={getPortPosition(connectionDraft.sourceNodeId, connectionDraft.sourcePortId, 'output').x}
                            y1={getPortPosition(connectionDraft.sourceNodeId, connectionDraft.sourcePortId, 'output').y}
                ...
                <div className={`pointer-events-auto ${readOnly ? 'pointer-events-none' : ''}`}> 
                    {nodes.map(node => {
                        const isConnected = activeConnection && (activeConnection.sourceNodeId === node.id || activeConnection.targetNodeId === node.id);
                    ...
                    {tooltip.type === 'image' ? (
                        ...
                        <div className="text-xs text-white max-w-[200px] break-all">
                            {typeof tooltip.content === 'object' 
                                ? JSON.stringify(tooltip.content).slice(0, 100) + (JSON.stringify(tooltip.content).length > 100 ? '...' : '') 
            ...
            {!readOnly && (
                <div className="absolute bottom-4 right-4 bg-gray-800/80 p-2 rounded text-xs text-gray-400 pointer-events-none">
                    Zoom: {Math.round(viewport.zoom * 100)}% | Pos: {Math.round(viewport.x)}, {Math.round(viewport.y)}
                </div>
            )}
            ...
            {logViewerNodeId && (
                <LogModal nodeId={logViewerNodeId} onClose={() => setLogViewerNodeId(null)} />
            )}
            ...
            <DetailPanel 
                ...
                onSelectConnection={(id) => { setSelectedConnectionId(id); handleSelectionChange(null); }}
                onClose={() => { handleSelectionChange(null); setSelectedConnectionId(null); }}
            />
        </div>
    </div>
  );
