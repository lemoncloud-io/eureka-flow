...
import { NodeData, Connection, BlockDefinition, ConfigField, DataPacket, ConfigControlType } from '../types';
    ...
    return (
        <div className="mt-1 relative group">
            ...
            <div className="absolute bottom-1 right-1 flex gap-1 pointer-events-none">
                ...
                <div className="bg-blue-900/80 text-[9px] px-1.5 py-0.5 rounded text-blue-100 backdrop-blur-md border border-blue-700/50 font-bold shadow-sm">IMG</div>
            </div>
...
export const DetailPanel: React.FC<DetailPanelProps> = ({
         ...
         case 'boolean':
             return (
                 <div className="flex items-center gap-2 mt-1">
                     ...
                     >
                         <div className={`w-3 h-3 bg-white rounded-full shadow-sm transition-transform ${value ? 'translate-x-4' : 'translate-x-0'}`} />
                     </div>
                            ...
                            <input 
                                ...
                                onChange={(e) => {
                                    ...
                                    if(file) {
                                        ...
                                        reader.onload = (evt) => evt.target?.result && handleChange(evt.target.result);
  ...
  if (selectedNode) {
    ...
    const configSchema = def.configSchema || Object.entries(def.defaultConfig).map(([key, value]): ConfigField => {
             ...
             <div className="flex flex-col gap-2">
                 <div className="flex items-center justify-between text-[10px] text-gray-400">
                     <div className="flex items-center gap-2">
                         <span className="font-mono bg-black/30 px-1 rounded border border-gray-700" title="Node Type">{def.type}</span>
                         <span className="font-mono text-gray-500 select-all" title="Node ID">#{selectedNode.id}</span>
                         <span className={`px-1.5 py-0.5 rounded border font-semibold ${
                            selectedNode.status === 'ERROR' ? 'border-red-500 text-red-400 bg-red-900/20' :
                            selectedNode.status === 'RUNNING' ? 'border-yellow-500 text-yellow-400 bg-yellow-900/20' :
                            selectedNode.status === 'COMPLETED' ? 'border-green-500 text-green-400 bg-green-900/20' :
                 ...
                 {isComponent && subFlowId && (
                     <div className="flex items-center gap-2 text-[10px] bg-indigo-900/30 px-2 py-1.5 rounded border border-indigo-500/30">
                        <span className="uppercase font-bold text-indigo-300 opacity-70 tracking-wider">Sub-Flow ID</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <div>
                <div className="flex items-center justify-between mb-2">
                    ...
                    {/* Auto Toggle */}
                     ...
                     >
                        <span className={`text-[9px] font-semibold ${isAuto ? 'text-green-400' : 'text-gray-500'}`}>Auto</span>
                        <div className={`w-6 h-3 rounded-full p-0.5 transition-colors relative ${isAuto ? 'bg-green-600/80' : 'bg-gray-700'}`}>
                            <div className={`w-2 h-2 bg-white rounded-full shadow-sm transition-transform absolute top-0.5 ${isAuto ? 'left-[calc(100%-10px)]' : 'left-0.5'}`} />
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-3 bg-gray-800/30 p-3 rounded border border-gray-800">
                    {configSchema.length === 0 ? (
                        ...
                        configSchema.map(field => (
                            <div key={field.key}>
                                <label className="text-[10px] text-gray-400 font-semibold mb-1 block">{field.label}</label>
            ...
            <div>
                ...
                <div className="space-y-3">
                    {def.inputs.length === 0 ? <div className="text-xs text-gray-600 italic">No inputs required</div> :
                     def.inputs.map(input => {
                        // Find connection feeding this input
                        const incomingConn = connections.find(c => c.targetNodeId === selectedNode.id && c.targetPortId === input.id);

                        return (
                            <div key={input.id} className="bg-gray-800/30 p-2 rounded border border-gray-800 relative">
                                <div className="flex justify-between items-center mb-1">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs font-bold text-gray-300">{input.label}</span>
                                        {incomingConn && (
                                            <button 
                                    ...
                                    <span className="text-[9px] text-gray-500 uppercase border border-gray-700 px-1 rounded">{input.type}</span>
                                </div>
            ...
            <div>
                ...
                <div className="space-y-3">
                    {def.outputs.length === 0 ? <div className="text-xs text-gray-600 italic">No outputs</div> :
                     def.outputs.map(output => {
                         // Find connections going out from this output
                         const outgoingConns = connections.filter(c => c.sourceNodeId === selectedNode.id && c.sourcePortId === output.id);

                         return (
                            <div key={output.id} className="bg-gray-800/30 p-2 rounded border border-gray-800">
                                 <div className="flex justify-between items-center mb-1">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs font-bold text-gray-300">{output.label}</span>
                                    ...
                                    <span className="text-[9px] text-gray-500 uppercase border border-gray-700 px-1 rounded">{output.type}</span>
                                </div>
             ...
             <div className="p-4 space-y-6">
                 ...
                 <div className="flex flex-col items-center gap-2 relative">
                     ...
                     >
                         ...
                         <div className="font-bold text-xs text-blue-300 group-hover:text-white">{sourceDef?.label || 'Unknown'}</div>
                     ...
                     <div className="w-full bg-black/40 p-2 rounded border border-dashed border-gray-600 relative">
                         <div className="text-[9px] text-center text-gray-500 mb-1 uppercase tracking-wider">Payload</div>
                     ...
                     >
                         ...
                         <div className="font-bold text-xs text-green-300 group-hover:text-white">{targetDef?.label || 'Unknown'}</div>
