  ...
  return (
    <div className="h-14 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-4 z-30 shadow-md shrink-0">
      ...
      <div className="flex items-center gap-4">
        ...
        >
            <div className={`w-8 h-4 rounded-full p-0.5 transition-colors relative ${isAutoSaveEnabled ? 'bg-green-600/80' : 'bg-gray-700'}`}>
                <div className={`w-3 h-3 bg-white rounded-full shadow-sm transition-transform absolute top-0.5 ${isAutoSaveEnabled ? 'left-[calc(100%-14px)]' : 'left-0.5'}`} />
            </div>
            <span className={`text-xs font-semibold select-none ${isAutoSaveEnabled ? 'text-green-400' : 'text-gray-500 group-hover:text-gray-400'}`}>
                Auto Save
            </span>
        </div>
